    ...
    >
        <div class="bg-white w-[900px] rounded-3xl shadow-xl p-5 px-7 relative">
            ...
            >
                ...
                />
                <label for="class_date" class="flex flex-col gap-2">
                    <span>Class date:</span>
                    <input
                        type="date"
                        v-model="info.class_date"
                        :min="minDate"
                        id="class_date"
...
const handleSubmit = () => {
    ...
    const selectedDate = new Date(info.value.class_date);
    ...
    error.value = null;
    store.dispatch("CreateClassSession", info.value).then((res) => {
        if (res.success) {
