<template>
    <form @submit.prevent="handleSubmit" class="w-full grid grid-cols-3 gap-4">
        <span class="text-2xl mb-4">Create student</span>
        <ErrorComp
            v-if="error"
            :message="error"
            class="col-span-full"
        ...
        <label for="grade" class="flex flex-col gap-2">
            <span>Grade:</span>
            <input
                type="number"
                id="grade"
...
const validateForm = () => {
    ...
    const validationRules = [
        ...
        },
        {
            ...
            message:
                "Invalid academic level. Please select a valid academic level.",
        },
        {
    ...
    for (const rule of validationRules) {
        ...
        if (rule.meta) {
            check = rule.condition(
                stdInfo.value[rule.field],
                rule.meta.obj,
                rule.meta.key
            );
        } else {
            check = rule.condition(stdInfo.value[rule.field]);
        }
...
const handleSubmit = () => {
    ...
    if (validateForm()) {
        const data = convertToFormData(stdInfo.value);
        data.append("picture", stdPictue.value);
        store.dispatch("createStudent", data).then((res) => {
            if (res.student) {
                successMsg.value = "Student created successfuly!";
                resetValues();
            }
        });
    }
